...
func (a *apiServer) GetLastCommitTree(repoName, branchName) ([]*File, error){
  returnn nil, fmt.Errorf("unimplemented")
}
...
func (a *apiServer) GetCommitTree(repoName, branchName, commitId string, search_path string) (results map[string][]*File, fnerr error)
  ...
  file_map := a.GetCommitMap(repoName, commitId)
