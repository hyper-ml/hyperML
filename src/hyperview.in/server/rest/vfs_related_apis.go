...
func (h *handler) handleListDir() error {
  ...
  info_map := &ws.FileInfoMap{Entries: finfo_map}
...
func (h *handler) handleVfsPutFile() error {
  ...
  if object_hash == "" {
    ...
    file_info, written, err = h.server.workspaceApi.PutFile(repo_name, commit_id, file_path, h.rq.Body) 
  } else {
  ...
  response = map[string]interface{}{
    "file_info" : structs.Map(file_info),
    "written" : written,
  }
  h.writeJSON(response)
  return nil
