...
func (h *Handler) handleListDir() error {
  ...
  finfo_map, err := h.server.vfs.ListDir(repo_name, branch_name, commit_id, dir_path)
  if err != nil {
    base.Log("Failed to retrieve file in dir: %s %s %s", repo_name, branch_name, commit_id, dir_path)
    return err
  }
...
func (h *Handler) handleFileLookup() error {
  ...
  base.Debug("[Handler.handleFileLookup] ", repo_name, branch_name, commit_id, fpath)
  ...
  if branch_name == "" { 
    base.Debug("[Handler.handleFileLookup] No branch_name passed in request")
  }
  ...
  finfo, err := h.server.vfs.Lookup(repo_name, branch_name, commit_id, fpath)
...
func (h *Handler) handleVfsPutFile() error {
  ...
  if object_hash == "" {
    ...
    file_attrs, written, err = h.server.workspaceApi.PutFile(repo_name, branch_name, commit_id, file_path, h.rq.Body) 
  } else {
