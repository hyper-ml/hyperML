...
func (f *file) Attr(ctx context.Context, a *fuse.Attr) (funcErr error) {
  ...
  file_info, err := f.fs.wc.FetchFileInfo(f.RepoName, f.WsFile.Commit.Id, f.WsFile.Path)

  if err != nil {
    return err
  }
 
  a.Size = uint64(file_info.SizeBytes)
...
func (f *file) Open(ctx context.Context, req *fuse.OpenRequest, resp *fuse.OpenResponse) (_ fs.Handle, retErr error) {
  ...
  file_info, err := f.fs.wc.FetchFileInfo(f.WsFile.Commit.Repo.Name, f.WsFile.Commit.Id, f.WsFile.Path)
  if err!= nil {
    base.Log("Open: Unable to fetch file info from server:", err)
    return nil, err
  }

  h := f.newFileHandle(int(file_info.SizeBytes))
  return h, nil
}
